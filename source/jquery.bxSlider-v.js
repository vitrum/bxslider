...
(function($){
	...
	$.fn.bxSlider = function(options){		
		...
		var animateTime= Number(options.speed)/1000;
		...
		this.goToSlide = function(number, stopAuto){
			if(!isWorking){
				...
				if(options.mode == 'horizontal'){
					try {
						//TweenMax
						TweenLite.to( $parent, animateTime , {css:{'left': '-'+getSlidePosition(slide, 'left')+'px'}, onComplete:function(){
							isWorking = false;
				...
				// vertical
					...
					try {
						//TweenMax
						TweenLite.to( $parent, animateTime , {css:{'top': '-'+getSlidePosition(slide, 'top')+'px'}, onComplete:function(){
							isWorking = false;
							// perform the callback function
							options.onAfterSlide(currentSlide, $children.length, $children.eq(currentSlide));
						}})
